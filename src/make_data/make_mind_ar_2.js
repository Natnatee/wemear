export const make_mind_ar = [
  {
    id: "4dce27a0-486c-4d87-a0b7-7c6b66dd210e",
    created_at: "2025-09-14T16:16:56.29961+00:00",
    "image tracking": {
      track1: {
        image:
          "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/public/assets/AS0101_pamh6b.jpg",
        scene: {
          scene_1: [
            {
              src: "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/public/assets/AS0101_pamh6b.jpg",
              type: "Image",
              scale: [0.2, 0.2, 0.2],
              opacity: 1,
              position: [0, 0, 0],
              rotation: [0, 0, 0],
            },
          ],
        },
      },
      track2: {
        image:
          "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/public/assets/flower.mp4",
        scene: {
          scene_1: [
            {
              src: "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/public/assets/flower.mp4",
              loop: true,
              type: "Video",
              muted: true,
              scale: [0.3, 0.3, 0.3],
              autoplay: true,
              position: [0, 0, 0],
              rotation: [0, 0, 0],
            },
          ],
        },
      },
      track3: {
        image:
          "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/public/assets/AS0101_pamh6b.jpg",
        scene: {
          scene_1: [
            {
              src: "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/assets/uploads_files_2882496_Shine+Sprite.gltf",
              type: "3D Model",
              scale: [0.1, 0.1, 0.1],
              position: [0, 0, 0],
              rotation: [0, 0, 0],
            },
          ],
        },
      },
      track4: {
        image:
          "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/public/assets/AS0101_pamh6b.jpg",
        scene: {
          scene_1: [
            {
              src: "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/assets/uploads_files_2882496_Shine+Sprite.gltf",
              type: "3D Model",
              scale: [0.1, 0.1, 0.1],
              position: [0.5, 0, 0],
              rotation: [0, 0, 0],
            },
            {
              src: "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/public/assets/AS0101_pamh6b.jpg",
              type: "Image",
              scale: [0.2, 0.2, 0.2],
              opacity: 1,
              position: [0, 0, 0],
              rotation: [0, 0, 0],
            },
            {
              src: "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/public/assets/flower.mp4",
              loop: true,
              type: "Video",
              muted: true,
              scale: [0.3, 0.3, 0.3],
              autoplay: true,
              position: [-0.5, 0, 0],
              rotation: [0, 0, 0],
            },
          ],
          scene_2: [
            {
              src: "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/assets/uploads_files_2882496_Shine+Sprite.gltf",
              type: "3D Model",
              scale: [0.1, 0.1, 0.1],
              position: [0, 0, 0],
              rotation: [0, 0, 0],
            },
          ],
        },
      },
    },
    mindFile:
      "https://msdwbkeszkklbelimvaw.supabase.co/storage/v1/object/assets/final_2.mind",
  },
];
